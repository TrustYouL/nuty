...
+ (BOOL)customCreateZipFileAtPath:(NSString *)path withFilesAtDirectorys:(NSArray *)paths {
    return [SSZipArchive customCreateZipFileAtPath:path withFilesAtDirectorys:paths withPassword:nil andProgressHandler:nil];
}

+ (BOOL)customCreateZipFileAtPath:(NSString *)path withFilesAtDirectorys:(NSArray *)paths withPassword:(nullable NSString *)password andProgressHandler:(void(^ _Nullable)(NSUInteger entryNumber, NSUInteger total))progressHandler {
    BOOL success = NO;
    
    NSFileManager *fileManager = nil;
    SSZipArchive *zipArchive = [[SSZipArchive alloc] initWithPath:path];
    NSString *directoryPath = [TOPDocumentHelper top_appBoxDirectory];
    BOOL keepParentDirectory = NO;
    ...
    if ([zipArchive open]) {
        ...
        for (fileName in allObjects) {
            if ([fileName hasPrefix:@"Documents"] || [fileName hasPrefix:@"SignPng"] || [fileName hasPrefix:@"Folders"]|| [fileName hasPrefix:@"Tags"]) {
                BOOL isDir;
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password andProgressHandler:(void(^ _Nullable)(NSUInteger entryNumber, NSUInteger total))progressHandler {
    ...
    if ([zipArchive open]) {
        ...
        for (NSString* filename in tempAllObjects) {

            [self changeAndroidFileName:[directoryPath stringByAppendingPathComponent:filename]];

        }
        dirEnumerator = [fileManager enumeratorAtPath:directoryPath];
        tempAllObjects = dirEnumerator.allObjects;
        NSMutableArray *allObjects = [NSMutableArray arrayWithCapacity:42];
        NSString *filename;
        for (filename in tempAllObjects) {
            NSString *currentFilePath = [directoryPath stringByAppendingPathComponent:filename];

            if (isSaveOriginalImage == YES) {
                if ([filename hasPrefix:@"Documents"] || [filename hasPrefix:@"SignPng"] || [filename hasPrefix:@"Folders"]|| [filename hasPrefix:@".Tags"]) {
                    if ([self isDirectory:currentFilePath]) {
                        NSString *fileChangeTime = [self top_getModifyTimeString:currentFilePath];
                        [TOPDocumentHelper top_extended2WithPath:currentFilePath key:TOP_TRFileADDCustomExtentedKey value:fileChangeTime];
                        NSDictionary *attributes = [[NSFileManager defaultManager] attributesOfItemAtPath:currentFilePath error:NULL];
                        NSLog(@"=========%@",attributes);
            ...
            }else
            {
                ...
                if ([filename hasPrefix:@"Documents"] || [filename hasPrefix:@"SignPng"] || [filename hasPrefix:@"Folders"]|| [filename hasPrefix:@".Tags"]) {
                    if ([self isDirectory:currentFilePath]) {
                        ...
                        [TOPDocumentHelper top_extended2WithPath:currentFilePath key:TOP_TRFileADDCustomExtentedKey value:fileChangeTime];
                        NSDictionary *attributes = [[NSFileManager defaultManager] attributesOfItemAtPath:currentFilePath error:NULL];
                        NSLog(@"=========%@\n%@",attributes,currentFilePath);

                    }
                    if ([filename containsString:@"original_"]) {
                        continue;
                    }else{
                        if ([filename isEqualToString:@".DS_Store"]) {
                            continue;
...
+ (BOOL)sourceCodeAddZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath {
    return [self sourceCodeCreateZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:YES withPassword:nil andProgressHandler:nil];
}

+ (BOOL)sourceCodeCreateZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password andProgressHandler:(void(^ _Nullable)(NSUInteger entryNumber, NSUInteger total))progressHandler {
    BOOL success = NO;
...
//    NSLog(@"%@",tempTags);
    ...
    if (isExist) {
        if ([tempTags.lastPathComponent hasPrefix:@"Tags"]) {
            tempTags =  [TOPDocumentHelper top_changeDocumentName:tempTags folderText:[@"." stringByAppendingString:tempTags.lastPathComponent]];
        }
        if ([tempTags.lastPathComponent hasPrefix:@"DocPassword_"]) {
            tempTags =  [TOPDocumentHelper top_changeDocumentName:tempTags folderText:[@"." stringByAppendingString:tempTags.lastPathComponent]];
        }
//         NSMutableArray *folderList = [TOPDocumentHelper top_getCurrentFileAndPath:tempTags];
        ...
        NSArray *folderList = [fileManger contentsOfDirectoryAtPath:tempTags error:&error];
         for (NSString *fileName in folderList) {
             if ([tempTags.lastPathComponent isEqualToString:@".Tags"]) {
                 if ([fileName isEqualToString:@".DS_Store"]) {
                     continue;
                 }else{
                     [TOPDocumentHelper top_changeFileName:[tempTags stringByAppendingPathComponent:fileName] folderText:[@"." stringByAppendingString:fileName]];

                 }
             }else{
                 if ( [fileName hasPrefix:@"original_"] ) {
                      [TOPDocumentHelper top_changeFileName:[tempTags stringByAppendingPathComponent:fileName] folderText:[@"." stringByAppendingString:fileName]];
                 }
