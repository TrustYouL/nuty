...
public class TOPCropView: UIView, UIGestureRecognizerDelegate {
    //MARK:Public Variables
    public var rectangleBorderColor = UIColor(red: 36/255.0, green: 196/255.0, blue: 164/255.0, alpha: 1.0)
    public var rectangleFillColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.5)
    public var circleBorderColor =  UIColor(red: 36/255.0, green: 196/255.0, blue: 164/255.0, alpha: 1.0)
    public var circleBackgroundColor = UIColor(red: 255/255.0, green: 255/255.0, blue: 255/255.0, alpha: 1.0)
    public var selectedCircleBorderColor = UIColor(red: 36/255.0, green: 196/255.0, blue: 164/255.0, alpha: 1.0)
    public var selectedCircleBackgroundColor = UIColor(red: 255/255.0, green: 255/255.0, blue: 255/255.0, alpha: 0.3)
    
    public var rectangleBorderWidth:CGFloat = 1.0
    ...
    public func setUpImage(image : UIImage,isAutomatic:Bool){
        if (image.size.width > 0 && image.size.height > 0){
            ...
            if(!self.subviews.contains(cropImageView)){
                cropImageView = UIImageView(image: top_normalizedImage(image: image))
                cropImageView.contentMode = .scaleAspectFit
                cropImageView.frame = top_adaptiveImageFrame(image: image)
                self.addSubview(cropImageView)
                cropFrame = cropImageView.frame
                top_setUpCropRegion()
                top_setUpGestureRecognizer()
            } else {
                //删除当前的试图
                ...
                cropImageView.image = self.top_normalizedImage(image: image)
                cropImageView.frame = top_adaptiveImageFrame(image: image)
                cropFrame = cropImageView.frame
                top_setUpCropRegion()
            }
...
//        print(cropImageView.frame, corners, cropImageView.bounds)
        let topWidth = top_distanceBetweenPoints(point1: corners[0], point2: corners[1])
        let bottomWidth = top_distanceBetweenPoints(point1: corners[3], point2: corners[2])
        let leftHeight = top_distanceBetweenPoints(point1: corners[0], point2: corners[3])
        let rightHeight = top_distanceBetweenPoints(point1: corners[1], point2: corners[2])
...
//        let newImage = TOPOpenCVWrapper.getTransformedImage(newWidth*widthScale, newHeight*heightScale, cropImageView.image, &corners2, (cropImageView.image!.size))
        let newImage = TOPOpenCVWrapper.top_getTransformedImage(newWidth*widthScale, newHeight*heightScale, originalImage, &corners2, (originalImage!.size))
        return newImage!;
    }
     ...
     */
    private func top_setUpCropRegion(){
        ...
        maskLayer.lineWidth = rectangleBorderWidth
...
//            print("img =","siz==",cropImageView.image as Any,cropImageView.frame.size);
            if (autoCropPoints.count > 0) {
                ...
                let points = TOPOpenCVWrapper.top_getLargestSquarePoints(cropImageView.image, cropImageView.frame.size,isAutoMax)
        ...
        while(i<=8){
            ...
            case 2,4,6,8:
                ...
                cropCircle.center = top_centerOf(firstPoint: endPoints[(i/2)-1], secondPoint: endPoints[i == 8 ? 0 : i/2])
                cropCircle.layer.addSublayer(circleLayer)
        ...
        top_adjustCornerPointsAngle()
        top_redrawBorderRectangle()
    }
    
    /**
     Draw/Redraw the crop rectangle such that it passes through the corner points
     */
    private func top_redrawBorderRectangle(){
        //剪裁区
 ...
 */
    private func top_setUpGestureRecognizer(){
        let gestureRecognizer = UIPanGestureRecognizer.init(target: self, action: #selector(TOPCropView.panGesture))
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        ...
        let index = top_getClosestCorner(point: point)
        if index == -1 {
            return false
        }
        return true
    ...
    @objc internal func panGesture(gesture : UIPanGestureRecognizer){
        ...
        if(gesture.state == UIGestureRecognizer.State.began){
            selectedIndex = top_getClosestCorner(point: point)
            if selectedIndex == -1 {
                return
            }
            oldPoint = point
        ...
        if gesture.state == UIGestureRecognizer.State.changed {
            ...
            if let selectedIndex = selectedIndex {
                if((selectedIndex) % 2 != 0){//通过手势平移边缘点-移动对应的边缘平行于它的旧位置
                    ...
                    if(top_isInsideFrame(pt: pt1New)){
                        pt1.center = pt1New
                    }
                    if top_isInsideFrame(pt: pt2New) {
                        pt2.center = pt2New
                    }
...
//                    print("pt1New=", pt1New, "pt2New=", pt2New, cropImageView.frame.origin.x)
     ...
     */
    private func top_normalizedImage(image: UIImage) -> UIImage {
    ...
    private func top_adaptiveImageFrame(image: UIImage) -> CGRect {
     ...
     */
    private func top_reorderEndPoints(){
     ...
     */
    private func top_getClosestCorner(point: CGPoint) -> Int{
        var index = -1
        var minDistance = self.touchRange > 0 ? self.touchRange : CGFloat.greatestFiniteMagnitude
        for i in stride(from: 0, to: 8, by: 1){
            let distance = top_distanceBetweenPoints(point1: point, point2: cropCircles[i].center)
    ...
    private func top_moveNonCornerPoints(){
    ...
    //MARK: 调整中间点的角度和变形平行
    private func top_adjustCornerPointsAngle() {
        for i in stride(from: 1, to: 8, by: 2){
            let prev = i-1
            let next = (i == 7 ? 0 : i+1)
            switch i{
            case 1,5:
                let rad = top_angleBetweenPoints(point1: cropCircles[prev].center, point2: cropCircles[next].center)
    ...
    private func top_isInsideFrame(pt: CGPoint) -> Bool{
    ...
    private func top_checkIfOppositeSides(p1:CGPoint, p2: CGPoint, l1: CGPoint, l2:CGPoint) -> Bool{
    ...
    private func top_checkQuadrilateral(){
        let A = cropCircles[0].center
        let B = cropCircles[2].center
        let C = cropCircles[4].center
        let D = cropCircles[6].center
        
        if(top_checkIfOppositeSides(p1: B,p2: D,l1: A,l2: C) && top_checkIfOppositeSides(p1: A,p2: C,l1: B,l2: D)){//Convex
            border.strokeColor = rectangleBorderColor.cgColor
        }else if(!top_checkIfOppositeSides(p1: B,p2: D,l1: A,l2: C) && !top_checkIfOppositeSides(p1: A,p2: C,l1: B,l2: D)){//Complex
            border.strokeColor = rectangleBorderColor.cgColor
            top_reorderEndPoints()
        } else{//Concave
    ...
    private func top_distanceBetweenPoints(point1: CGPoint, point2: CGPoint) -> CGFloat{
    ...
    private func top_centerOf(firstPoint: CGPoint, secondPoint: CGPoint) -> CGPoint{
        return CGPoint(x: (firstPoint.x+secondPoint.x)/2, y: (firstPoint.y + secondPoint.y)/2)
    }
    
    ///Returns the center of two CGPoints
    private func top_angleBetweenPoints(point1: CGPoint, point2: CGPoint) -> CGFloat{
        //两个直角边
    ...
    @objc
    public func top_saveChangeEndPointArray() ->[NSValue]{
