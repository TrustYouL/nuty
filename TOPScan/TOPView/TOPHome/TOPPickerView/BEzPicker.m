...
- (void)initialize
{
    self.font = self.font ?: [UIFont fontWithName:@"HelveticaNeue-Light" size:13];
    self.highlightedFont = self.highlightedFont ?: [UIFont fontWithName:@"HelveticaNeue" size:17];
    self.textColor = self.textColor ?: kWhiteColor;
    self.highlightedTextColor = self.highlightedTextColor ?: kTopicBlueColor;
    ...
    UIView *tintMarkView = [[UIView alloc] init];
    tintMarkView.backgroundColor = kTopicBlueColor;
    tintMarkView.layer.cornerRadius = 1.0;
    [self addSubview:tintMarkView];
    [tintMarkView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (CGSize)highlightedSizeForString:(NSString *)string
{
    CGSize highlightedSize;
#ifdef __IPHONE_7_0
    highlightedSize = [string sizeWithAttributes:@{NSFontAttributeName: self.highlightedFont}];
...
//        NSInteger item = scrollView.contentOffset.x > _offer ? self.selectedItem + 1 : self.selectedItem -1;
//        item = MIN(MAX(0, item), 4) ;
//        [self.collectionView setContentOffset:CGPointMake([self offsetForItem:item], self.collectionView.contentOffset.y)
//                                     animated:NO];
////        _offer = self.collectionView.contentOffset.x;
//    }
}
