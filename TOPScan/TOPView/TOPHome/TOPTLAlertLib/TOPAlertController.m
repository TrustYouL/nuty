...
- (void)updatePreferredContentSizeWithTraitCollection:(UITraitCollection *)traitCollection {
    ...
    CGFloat W = _preferredStyle == TOPAlertControllerStyleAlert ? kAlertWidth : kMaxWidth; // 始终以最小屏宽计算
...
- (void)addAction:(TOPAlertAction *)action {
    ...
    if (![action isKindOfClass:[TOPAlertAction class]]) {
        [NSException raise:@"TOPAlertController `-addAction:`使用错误" format:@"添加的TOPAlertAction必须是TOPAlertAction或其子类"];
    }
    if (action.style == TOPAlertActionStyleCancel) {
        if (_cancelAction) {
            [NSException raise:@"TOPAlertController使用错误" format:@"同一个alertController不可以同时添加两个cancel按钮"];
        }else {
...
- (void)showInViewController:(UIViewController *)vc {
    if (self.actions.count == 0) {
        [NSException raise:@"TOPAlertController 使用错误" format:@"actions的个数不能小于1"];
    }
