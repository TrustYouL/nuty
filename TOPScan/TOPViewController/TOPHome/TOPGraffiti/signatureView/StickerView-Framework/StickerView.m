...
#define kStickerControlViewSize 24
#define kStickerHalfControlViewSize 12

#define kStickerMinScale 0.5f
#define kStickerMaxScale 4.0f
...
- (instancetype)initWithContentFrame:(CGRect)frame contentImage:(UIImage *)contentImage {
    if (isnan(frame.size.width) || isnan(frame.size.height) || isnan(frame.origin.x) || isnan(frame.origin.y)) {
        CGRect tempFrame = CGRectZero;
        tempFrame.size.width = 75;
    ...
    if (self) {
        ...
        self.resizeControl.image = [UIImage imageNamed:@"top_signature_resize"];
        ...
        self.deleteControl.image = [UIImage imageNamed:@"top_signature_delete"];
...
- (void)setContrlShadow {
    self.resizeControl.layer.shadowColor = RGBA(8, 29, 41, 0.5).CGColor;
    // 设置阴影偏移量
    self.resizeControl.layer.shadowOffset = CGSizeMake(0,1);
    // 设置阴影透明度
    self.resizeControl.layer.shadowOpacity = 1;
    // 设置阴影半径
    self.resizeControl.layer.shadowRadius = 3;
    self.resizeControl.clipsToBounds = NO;
    
    self.rightTopControl.layer.shadowColor = RGBA(8, 29, 41, 0.5).CGColor;
    // 设置阴影偏移量
    self.rightTopControl.layer.shadowOffset = CGSizeMake(0,1);
    // 设置阴影透明度
    self.rightTopControl.layer.shadowOpacity = 1;
    // 设置阴影半径
    self.rightTopControl.layer.shadowRadius = 3;
    self.rightTopControl.clipsToBounds = NO;
    
    self.deleteControl.layer.shadowColor = RGBA(8, 29, 41, 0.5).CGColor;
    // 设置阴影偏移量
    self.deleteControl.layer.shadowOffset = CGSizeMake(0,1);
    // 设置阴影透明度
    self.deleteControl.layer.shadowOpacity = 1;
    // 设置阴影半径
    self.deleteControl.layer.shadowRadius = 3;
    self.deleteControl.clipsToBounds = NO;
}
...
- (void)initShapeLayer {
    ...
    [_shapeLayer setStrokeColor:[kTopicBlueColor CGColor]];
    
    
    [_shapeLayer setLineWidth:1.0];
    [_shapeLayer setLineJoin:kCALineJoinRound];
    _shapeLayer.allowsEdgeAntialiasing = YES;
    [_shapeLayer setLineDashPattern:[NSArray arrayWithObjects:[NSNumber numberWithInt:5], [NSNumber numberWithInt:0], nil]];
...
- (void)setupConfig {
    ...
    self.enabledMove = YES;
}

- (void)setEnabledInteraction:(BOOL)enabledInteraction {
    _enabledInteraction = enabledInteraction;
    self.userInteractionEnabled = _enabledInteraction;
}
...
- (void)attachGestures {
    ...
    // DeleteControl
    UITapGestureRecognizer *tapRecognizer2 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(top_handleTap:)];
    ...
    // RightTopControl
    UITapGestureRecognizer *tapRecognizer3 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(top_handleTap:)];
    ...
    // LeftBottomControl
    UITapGestureRecognizer *tapRecognizer4 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(top_handleTap:)];
...
- (void)top_handleTap:(UITapGestureRecognizer *)gesture {
    if (gesture.view == self.contentView) {
        ...
        if (_enabledDeleteControl) {
            ...
            if (_delegate && [_delegate respondsToSelector:@selector(top_stickerViewDidTapDeleteControl:)]) {
                [_delegate top_stickerViewDidTapDeleteControl:self];
            }
        }
    } else if (gesture.view == self.rightTopControl) {
        if (_enableRightTopControl) {
            if (_delegate && [_delegate respondsToSelector:@selector(top_stickerViewDidTapRightTopControl:)]) {
                [_delegate top_stickerViewDidTapRightTopControl:self];
            }
        }
...
- (void)handleTapContentView {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(top_stickerViewDidTapContentView:)]) {
        [_delegate top_stickerViewDidTapContentView:self];
    }
}
...
- (void)handleMove:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateEnded) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(stickerViewDidMoveEnd:)]) {
            [self.delegate stickerViewDidMoveEnd:self];
        }
    } else if (gesture.state == UIGestureRecognizerStateChanged) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(top_stickerViewBeMoving:withPoint:)]) {
            [self.delegate top_stickerViewBeMoving:self withPoint:targetPoint];
        }
    }
}
...
- (void)handleRotate:(UIRotationGestureRecognizer *)gesture {
    [self rotateContentView:gesture.rotation];
    gesture.rotation = 0;
...
- (void)rotateContentView:(CGFloat)rotation {
    self.contentView.transform = CGAffineTransformRotate(self.contentView.transform, rotation);
}
...
- (void)relocalControlView {
    ...
    if (isnan(originalCenter.x) || isnan(originalCenter.y)) {
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    if ([gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]] || [otherGestureRecognizer isKindOfClass:[UITapGestureRecognizer class]]||([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]] && self.enabledMove)) {
        return NO;
    } else {
        return YES;
    }
