...
@interface TOPTagsCellSpaceFlowLayout(){
    CGFloat _sumCellWidth ;
}
@end
@implementation TOPTagsCellSpaceFlowLayout
-(instancetype)init{
...
-(instancetype)initWithType:(AlignType)cellType betweenOfCell:(CGFloat)betweenOfCell{
    ...
    if (self){
        ...
        self.minimumInteritemSpacing = betweenOfCell;
        self.sectionInset = UIEdgeInsetsMake(20, 5, 20, 5);
        _betweenOfCell = betweenOfCell;
        _cellType = cellType;
    }
    return self;
    ...
    switch (_cellType) {
        case AlignWithLeft:
            ...
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                ...
                if (nowFrame.size.width>=TOPScreenWidth-self.betweenOfCell) {
                    nowFrame.size.width = TOPScreenWidth-self.betweenOfCell;
                }
        ...
        case AlignWithCenter:
            ...
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                ...
                if (nowFrame.size.width>=TOPScreenWidth-self.betweenOfCell) {
                    nowFrame.size.width = TOPScreenWidth-self.betweenOfCell;
                    nowFrame.origin.x = self.sectionInset.left;
                }else{
                    nowFrame.origin.x = nowWidth;
                }
                attributes.frame = nowFrame;
